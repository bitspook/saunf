...
module Saunf.Pandoc.Writer.Org (writeOrg) where
...
blockToOrg (LineBlock lns) = do
  ...
  return $
    blankline $$ "#+begin_verse"
      $$ nest 2 contents
      $$ "#+end_verse" <> blankline
blockToOrg (RawBlock "html" str) =
  return $
    blankline $$ "#+begin_html"
      $$ nest 2 (literal str)
      $$ "#+end_html"
      $$ blankline
...
blockToOrg (CodeBlock (_, classes, kvs) str) = do
  ...
  let numberlines =
        if "numberLines" `elem` classes
          then
            if "continuedSourceBlock" `elem` classes
              then " +n" <> startnum
              else " -n" <> startnum
          else ""
...
blockToOrg (OrderedList (start, _, delim) items) = do
  ...
  let markers' =
        map
          ( \m ->
              let s = maxMarkerLength - T.length m
               in m <> T.replicate s " "
          )
          markers
...
orderedListItemToOrg ::
  PandocMonad m =>
  -- | marker for list item
  Text ->
  -- | list item (list of blocks)
  [Block] ->
  Org m (Doc Text)
orderedListItemToOrg marker items = do
  ...
  contents <- blockListToOrg (taskListItemToOrg exts items)
  return $
    hang (T.length marker + 1) (literal marker <> space) contents
      $$ if endsWithPlain items
        then cr
        else blankline
...
propertiesDrawer (ident, classes, kv) =
  let drawerStart = text ":PROPERTIES:"
      drawerEnd = text ":END:"
   ...
   in drawerStart <> cr <> properties <> cr <> drawerEnd
  where
...
data DivBlockType
  = -- | Greater block like @center@ or @quote@.
    GreaterBlock Text Attr
  | -- | Org drawer with of given name; keeps
    --   key-value pairs.
    Drawer Text Attr
  | -- | Not mapped to other type, only
...
divToOrg attr bs = do
  ...
  case divBlockType attr of
    GreaterBlock blockName attr' ->
      ...
      return $
        blankline $$ attrHtml attr'
          $$ "#+begin_" <> literal blockName
          $$ contents
          $$ "#+end_" <> literal blockName
          $$ blankline
...
attrHtml (ident, classes, kvs) =
  let name = if T.null ident then mempty else "#+name: " <> literal ident <> cr
      keyword = "#+attr_html"
...
inlineListToOrg lst = hcat <$> mapM inlineToOrg (fixMarkers lst)
  where
    -- Prevent note refs and list markers from wrapping, see #4171
    -- and #7132.
    fixMarkers [] = []
    fixMarkers (Space : x : rest)
      | shouldFix x =
        Str " " : x : fixMarkers rest
    fixMarkers (SoftBreak : x : rest)
      | shouldFix x =
        Str " " : x : fixMarkers rest
...
inlineToOrg (Link _ txt (src, _)) =
  case txt of
    [Str x]
      | escapeURI x == src -> -- autolink
        return $ "[[" <> literal (orgPath x) <> "]]"
    _ -> do
      contents <- inlineListToOrg txt
