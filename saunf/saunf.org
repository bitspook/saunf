...
:CATEGORY: issues
:END:
** Add =fmt= as nix devshell command which format all source files   :chore:
...
** Add =lint= as nix devshell command to run lint on all source files :chore:
...
** Use cabal mixins technique to use Relude as default prelude       :chore:
